...
- **🔴 未推送的本地提交（2026-08-29，9 个）**：`0f4fbee`（捆绑 Python 收尾）、
  `2668065`（清理无用文件/整理文档）、`885ea83`（CI 修复 resources/python 校验）、
  `f2c9e06`（docs: 记录待推送提交与网络情况）、`d3c20c7`（安装依赖按钮 + 冒烟/打包验证收尾）、
  `9f71c8b`（Python 插件实现形式示例集）、`4594a58`（教学基线收敛：移除 6 个业务核心插件
  + core-example 教学示例 + 宿主文件服务迁回本体）、`cfa7394`（宿主可扩展性增强：
  顶栏动作/托盘菜单/设置页插件段）、`4f8874f`（process 核心 API 扩展 + 插件导出分享 +
  webview 最小示例 hello-tb）。
