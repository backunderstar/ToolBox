...
### 1.13b 文件处理全面化 + 插件文件上下文动作（2026-09，用户澄清定位后）

- **定位**（用户拍板）：宿主 = 文件处理基础能力（框架本体），插件 = 文件处理的
  表现形式与业务逻辑。根目录下多项目工作区，一切以当前工作区展开。
- **宿主文件能力补齐**：`files_copy`（文件/目录递归复制，目标已存在拒绝、防自拷贝）、
  `files_move`（语义化移动，内部 rename）；文件视图全面版：多选模式 + 批量删除/复制到…/
  移动到…、排序（名称/时间/大小升降序）、大小与相对时间展示、**右键菜单**（宿主操作 +
  插件动作）；`manifest actions` 增加 `file: true`（文件上下文动作，见下）。
- **插件文件上下文动作**（§7.1b 契约）：宿主文件视图右键/批量菜单显示 `file:true`
  动作 → `plugin.action {action, source:"file", files:[rel,...]}`（事件通道同 payload），
  插件读 `TB_WORKSPACE` + rel 执行组织逻辑（建结构/归类/转格式）。文件视图在动作
  返回后自动刷新。**越界防护是插件责任，协议测试有断言**。
- **示例**（probe-rat-layer）：`init-project-structure`（建 01-原始数据/02-报告/03-归档）、
  `archive-to-batch`（选中文件 → 03-归档/<日期>/，含 `_safe_join` 防逃逸）。
  protocol_test 新增第 11 节 3 项断言（含 `../` 越界被忽略），17 项全过。

### 1.13c 数据根目录模型重定义 + 首启引导 + 样式/菜单重做（2026-09，用户对 1.13 不满意后重做）

- **⚠️ 取代**：§1.13 的"根下直接子目录 = 工作区"模型废弃。用户重新定义：
  - **数据根目录**（root.json，%APPDATA%）:用户自定义一个目录存所有数据（如 D:\ToolBoxData）。
    根下按约定组织大项 `Project/`、`Plugin/`、`Config/`——**应用只管理 Project/**，
    插件仍装全局插件目录、配置仍在 %APPDATA%（用户拍板，代码与数据分离）。
  - **工作区 = 数据根/Project/<名称>**；日常选定工作区后文件处理（搜索/备份/文件/插件）
    都作用于当前工作区。
  - **每个工作区自动维护隐藏目录 `.toolbox`**（标记 + 配置/信息存放处）：宿主写
    `workspace.json` 元数据（name/created_at/updated_at；搜索索引/备份也落这里），
    插件可读写；文件视图隐藏。
  - 旧 vault.json **不再读取**（用户拍板不迁移，从零开始）；vault_get/vault_set 命令保留
    兼容，前端不再用。
- **首启引导页**（OnboardingView）：root 未配置 → 全屏引导（选数据根目录 + 选主题），
  配置完成自动进主界面；已配置 → 正常进入。App.vue 以 `showOnboarding` 分流。
- **样式重做**：文件视图全面对齐 tokens/标准视图风格（.view-header + .view-sub +
  .view-actions、卡片化列表、全部用设计令牌）；顶栏工作区下拉样式补齐（shell.css）。
- **右键菜单插件动作收二级**：一级菜单只留宿主操作；`file:true` 插件动作统一收进
  「插件处理 ▸」二级菜单（点击展开，按插件标注），批量操作条同理「插件处理 ▾」下拉
  ——插件再多也不占地方（用户反馈"插件多时不可能全显示"）。
...
### 1.16 增量（2026-09：走通率"真实可布"指标（连通分量洪泛）+ 算法深化基线）

- **真实可布走通率**（`post_process::routable_nets_flood`）：对分层后的每一层，在"容量内可走"
  栅格（supply>0 且 occupancy≤layer_capacity，keepout/超容格不可走）上做**4 邻接连通分量洪泛**，
  判定每条 net 的所有线端是否落在**同一连通可布区**（层内存在贯穿的、容量内的绕行通道）。
  比"直线/路径占用峰值≤容量"更诚实——走直线即使超容，只要层内存在容量内绕行通道就算可布。
  - 结果/报告/结果页新增 `routable_flood_net_count / routable_flood_ratio / unroutable_nets_flood`。
  - 新增单测 `flood_rejects_net_crossing_keepout_band`（禁布带把板分开 → 跨带 net 不可布、带外可布）。
  - ⚠️ 它是**只读诊断**（不改分层），纯测量，默认不影响现有结果/性能（hv 1800 网 1.86s）。
- **真实数据基线（hv，1800 网，hv 预设，cargo test --release -p ... -- --ignored --nocapture）**：
  `直线 1382/1798(77%) ｜ 路径 1435/1798(80%) ｜ **洪泛 1763/1798(98%)** ｜ 层占用峰值 1.78(>1.0)，
  各层 1.33–1.78 ｜ 硬冲突 272 ｜ 需人工 2 ｜ 用时 1.86s`。**结论**：分层本体可行（98% nets 可在
...
- **配置**：`congestion_balance`(bool, 默认 false) / `congestion_balance_passes`(int, 默认 20)。
  **UI**：`拥塞均衡` 开关 + 轮数 已加到「拥塞估计」卡片（含效果说明，默认关）。
