...
| 里程碑 | 内容 | 预估 |
|---|---|---|
| **M0 骨架** | Tauri 2 + TS/React 脚手架；Rust `ping` 命令打通 IPC；主题令牌 + 亮/暗切换；应用外壳布局（侧栏 + 主区 + 状态栏） | 0.5~1 天 |
| **M1 笔记** | 文件树 + CodeMirror 6 Markdown 编辑器 + 新建/保存/删除 + 全文搜索 + 最近打开 | 3~5 天 |
| **M2 插件系统 v1** | manifest 加载器；webview JS 插件 API；Python 进程桥（JSON-RPC）；热重载；示例插件 | 5~7 天 |
| **M3 数据工具** | JSON 格式化 / 时间戳转换 / Base64 / CSV↔JSON / 文本统计（内置 + 插件化） | 2~3 天 |
| **M4 清单与记录** | 清单（checklist）与工作记录（record）数据模型 + UI + 与笔记双向链接 | 3~4 天 |
| **M5 主题系统完整** | 主题包格式 + 切换器 + 主题编辑器；夜间模式打磨 | 2~3 天 |
| **M6 AI 集成** | 提供商配置（OpenAI 兼容 API）；对话/整理面板；选区摘要；笔记问答（RAG：嵌入 + 向量检索） | 1~2 周 |
| **M7 博客发布** | frontmatter → 导出管线；集成 Zola（Rust 静态博客生成器）一键生成/预览/发布 | 3~5 天 |
| 持续 | 插件仓库/市场、自动备份、设置面板、系统托盘、跨平台（macOS/Linux） | 长期 |
...
> 进度：M0 ✓（外壳/主题/ping）→ M1 ✓（笔记模块 + 设置页）→ M2 ✓（插件系统 v1，提交 53da90d）→ M3 ✓（数据工具，提交 e6d864f）。
