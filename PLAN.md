...
| ✅ 已完成 | **捆绑 Python 运行时（§1 风险提示"uv 嵌入式 Python"落地）** | 目标机无需装 Python 即可跑 process 插件：`pnpm fetch:python` 下载 python-build-standalone full 变体（3.14 + pip，SHA256 校验，`--mirror` 走镜像，瘦身 180→48.7MB）到 `src-tauri/resources/python/` 随 NSIS 分发；打包版首启 `ensure_bundled_python` 部署到 `%APPDATA%/com.toolbox.desktop/python/`（可写，为 pip install 留口子）；宿主解释器**三级解析**（`pyruntime::resolve_interpreter`）：插件目录自带 python.exe → 全局捆绑 → 系统 PATH；插件 stderr 改 piped 捕获（init 失败回显 Python traceback，缺依赖可读）；doctor/env:setup 增检查与自动下载；**数据对象不持有 tauri 类型**（PluginManager 存缓存路径而非 AppHandle，规避 0xC0000139 加载崩溃历史坑）。验证：cargo 81 测试、pnpm lint/test/build 全绿（dev 冒烟与打包 E2E 待补；插件页"安装依赖"按钮待做） |
