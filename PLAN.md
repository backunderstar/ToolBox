...
| ✅ 已完成 | **捆绑 Python 运行时（§1 风险提示"uv 嵌入式 Python"落地）** | 目标机无需装 Python 即可跑 process 插件：`pnpm fetch:python` 下载 python-build-standalone full 变体（3.14 + pip，SHA256 校验，`--mirror` 走镜像，瘦身 180→48.7MB）到 `src-tauri/resources/python/` 随 NSIS 分发；打包版首启 `ensure_bundled_python` 部署到 `%APPDATA%/com.toolbox.desktop/python/`（可写，为 pip install 留口子）；宿主解释器**三级解析**（`pyruntime::resolve_interpreter`）：插件目录自带 python.exe → 全局捆绑 → 系统 PATH；插件 stderr 改 piped 捕获（init 失败回显 Python traceback，缺依赖可读）；doctor/env:setup 增检查与自动下载；**数据对象不持有 tauri 类型**（PluginManager 存缓存路径而非 AppHandle，规避 0xC0000139 加载崩溃历史坑）；**插件页"安装依赖"按钮**（`plugins_install_deps` 命令：捆绑 python `pip install --target <插件>/vendor -r requirements.txt`，输出尾部回显 + 成功后自动重载插件；`PluginInfo.hasDeps` 标记有 requirements.txt 的插件，仅 process 插件显示按钮；py-tools 增 requirements.txt 示例）。验证：cargo 81 测试、pnpm lint/test/build 全绿，dev 冒烟 csv-tool/py-tools 均确认使用捆绑解释器（打包 E2E 见 §1.2 流程） |
| ✅ 已完成 | **教学基线收敛（2026-08-29，用户决策）** | 6 个业务核心插件（笔记/待办/清单/项目/博客/AI）全部移除（core-plugins/ 只剩 example），宿主引用同步清理：api.ts（fs* 改走宿主命令、删 ai*/todos* 封装）、vault.ts（精简为工作区/搜索/上下文快照）、navigation（ViewId 泛化，删 openNote/openChecklist）、App.vue（固定视图分支收敛）、FloatApp（页签 = core-example）、SettingsView（删 AI 段）、AISettings/blogfm 死代码删除、14 个 cdp-*.mjs E2E 脚本删除。**宿主文件服务迁回本体**（`core/files.rs`：files_list/read/write/create/delete/rename + S1c vault 校验；文件操作是插件系统与宿主共用的系统级能力，webview 桥与 process 核心 API 同源）。**教学示例 core-example**（cdylib + 自带前端，覆盖命令/事件/搜索提供者/宿主能力/配置/导航全部要点，教程 docs/核心插件示例教程.md）。验证：cargo 59 测试、pnpm lint/test/build 全绿、build:core 自检（自动清理旧随包插件）、dev 冒烟（5 个 process 插件用捆绑解释器 + core-example 部署正常） |
