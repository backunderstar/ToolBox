...
| **M8 项目文件管理** | 项目目录 + 归档；文件浏览器；点击用默认应用打开（方案：`docs/项目文件管理方案.md`） | 📋 方案待确认 |
...
| 优先级 | 事项 | 说明 |
|---|---|---|
| 后置（最后做） | **桌面半透明浮窗清单** | 方案已出：`docs/浮窗清单方案.md`；Tauri 多窗口 + 透明/置顶/位置记忆 + `data/todos.json` 纯文件真源；含 M-A~M-D 四阶段 |
...
| 待排期 | 自动备份 | vault 定时备份到 `.toolbox/backups/`（保留最近 N 份，可配置） |
