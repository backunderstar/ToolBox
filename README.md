...
- **M0 骨架（已完成）**：应用外壳（顶栏/侧栏/状态栏）、设计令牌与亮暗主题（含原生标题栏同步）、Rust `ping` 命令打通 IPC。
- **M1 笔记（已完成）**：Vault 工作区（文件夹选择 + 路径持久化）、Markdown 文件树（新建/重命名/删除）、CodeMirror 6 编辑器（自动保存 + Ctrl+S）、全文搜索（文件名 + 内容）、最近打开。
- 模块按里程碑推进：M2 插件系统 → M3 数据工具 → M4 清单与记录 → M5 主题系统 → M6 AI → M7 博客发布。
