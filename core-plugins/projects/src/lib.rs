//! 项目文件管理：vault/projects/ 下的项目文件夹、归档与默认应用打开。
//!
...
use tb_sdk::{resolve_relative, TbHostApi};
...
/// 项目列表（进行中 + 已归档，各带文件数）。
pub fn list(vault: &str) -> Result<Vec<ProjectInfo>, String> {
    let root = projects_root(vault);
    let mut out = Vec::new();
...
pub fn create(vault: &str, name: &str) -> Result<Value, String> {
    validate_project_name(name)?;
    let root = projects_root(vault);
...
pub fn archive(vault: &str, name: &str) -> Result<Value, String> {
...
pub fn unarchive(vault: &str, name: &str) -> Result<Value, String> {
...
pub fn delete(vault: &str, name: &str, permanent: bool) -> Result<Value, String> {
...
pub fn files(vault: &str, name: &str, dir: &str) -> Result<Vec<ProjectFile>, String> {
...
pub fn open(
    ...
    let Some(f) = host.open_path else {
        return Err("宿主未提供打开能力".to_string());
    };
    let path = CString::new(target.to_string_lossy().to_string()).map_err(|e| e.to_string())?;
    let rc = unsafe { f(ctx, path.as_ptr() as *const c_char) };
    if rc == 0 {
        Ok(Value::Null)
    } else {
        Err(format!("打开失败: {rel}"))
...
mod tests {
    ...
    fn tmp_vault(tag: &str) -> PathBuf {
        let p = std::env::temp_dir().join(format!("tb-projects-test-{tag}-{}", std::process::id()));
    ...
    fn archive_roundtrip() {
        ...
        assert!(!list.iter().any(|p| !p.archived && p.name == ARCHIVE_DIR));
        unarchive(vault, "A").unwrap();
    ...
    fn archive_name_collision() {
        ...
        let vault = v.to_str().unwrap();
        create(vault, "B").unwrap();
        archive(vault, "B").unwrap();
        create(vault, "B").unwrap();
        archive(vault, "B").unwrap();
    ...
    fn path_escape_rejected() {
        ...
        let vault = v.to_str().unwrap();
        create(vault, "P").unwrap();
        for bad in ["..", "../x", "a/../../x", "/abs", "C:/x"] {
            assert!(files(vault, "P", bad).is_err(), "应拒绝 rel: {bad:?}");
        }
        std::fs::remove_dir_all(&v).ok();
    }
    ...
    fn find_project_rejects_name_escape() {
        ...
        for bad in ["..", "../notes", "./x", "a\\b", "P/../notes"] {
            assert!(find_project(vault, bad).is_err(), "find_project 应拒绝: {bad:?}");
        }
    ...
    fn files_lists_dir() {
        ...
        let listing = files(vault, "P", "").unwrap();
        assert_eq!(listing.len(), 2);
        assert!(listing[0].is_dir && listing[0].name == "子目录", "目录应排前面");
        let f = listing.iter().find(|f| f.name == "a.txt").unwrap();
        assert_eq!(f.size, Some(5));
        let sub = files(vault, "P", "子目录").unwrap();
        assert!(sub.is_empty());
        std::fs::remove_dir_all(&v).ok();
    }
    ...
    fn delete_permanent() {
        ...
        let vault = v.to_str().unwrap();
        create(vault, "D").unwrap();
        delete(vault, "D", true).unwrap();
...
fn state_from_cfg(cfg: &Value) -> Result<ProjectsState, String> {
    let vault = cfg
        .get("vault")
        .and_then(|v| v.as_str())
        .unwrap_or("")
    ...
    Ok(ProjectsState { vault })
}

/// 命令分发。项目变更无跨窗口事件需求（仅项目页使用）。
fn call(
    ...
    match method {
        ...
        "projects.delete" => {
            ...
            let permanent = params
                ...
                .unwrap_or(false);
            delete(&vault, &name, permanent)
        }
        "projects.files" => {
            let name = s("name").ok_or("缺少 name")?;
            let dir = s("dir").unwrap_or_default();
            let out = files(&vault, &name, &dir)?;
            serde_json::to_value(out).map_err(|e| e.to_string())
        }
        "projects.open" => {
            let name = s("name").ok_or("缺少 name")?;
            let rel = s("rel").unwrap_or_default();
            open(&vault, &name, &rel, host, ctx)
        }
        _ => Err(format!("未知命令: {method}")),
    }
}

tb_plugin!(ProjectsState, state_from_cfg, call);
