...
> 状态：**已实施**（方案 A 已落地为 `core-plugins/probe-rat-layer/`，随 v0.3.0 发布；
> 本文件保留作**设计/决策记录与复盘**，见 §0.1 实际进度。下一步改进见
> [探针卡分层-可布线零过孔改进方案.md](探针卡分层-可布线零过孔改进方案.md)）。
> 原始目标：把 `plugins/probe-rat-layer` 的计算核心从 Python（含 130MB vendor）改写为
