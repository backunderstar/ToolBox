...
| 页签 | 内容 |
|---|---|
| **输入设置** | 输入 1：Allegro pin 表（.xls/.xlsx，兼容旧 JSON）；输入 2：筛选文件（可选，.lst/.txt 一行一个 net，兼容 .xls/.xlsx 第一列）；**输出目录（必填，强制指定）**；预设（**默认 HV** / 全量 / 自定义）；层数/线宽/线距。内置文件浏览器（盘符→目录→文件） |
| **分层参数** | 方法（packing/dsatur）、优化器（sa/greedy/none）、迭代①-④轮数、SA 参数、拥塞参数；未列字段用 `probe_layer` 默认值 |
| **运行** | 「开始分层」秒回（后台线程），进度条 + 阶段文案 + 事件流，可取消；自定义参数且阈值 <1.5 时提示 HV 预设 |
| **结果** | 摘要卡（层数/线数/硬软冲突/人工清单/耗时）、各层统计表（点击行→**按需渲染 SVG 图**，另有总览图/玫瑰图）、「查看 report.json」、.lst/CSV 查看与复制、「打开输出目录」；**人工线占比 >30% 时醒目提示并一键应用 HV 预设** |

> ⚠️ **默认预设 = HV（cell 2.0 / threshold 3.0）**：原项目文档明确默认 0.8/0.5 对 HV 太严
> （实测 1800 线 HV 全量用默认参数会 manual 1758 条；HV 预设 manual 仅 18 条）。
