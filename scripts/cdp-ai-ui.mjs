...
const presetsEnabled = await ev(
  `[...document.querySelectorAll('.plugin-ui-view .ai-presets button')].some(b => !b.disabled)`,
);
if (presetsEnabled) {
  // 宿主 vault 有当前笔记（如经笔记视图同步）→ 点"总结当前笔记"走预设流程（无 Key → 错误引导）
  await clickText(".plugin-ui-view .ai-presets button", "总结当前笔记");
  await waitFor(
    `[...document.querySelectorAll('.plugin-ui-view .ai-msg.ai-msg-user')].some(m => m.textContent.includes('请总结这篇笔记'))`,
    "预设动作用户消息出现",
  );
  await waitFor(
    `document.querySelectorAll('.plugin-ui-view .ai-msg.ai-msg-assistant.err').length >= 1`,
    "预设动作错误回复（无 API Key）",
  );
  log("PASS 预设动作可用（context 快照有当前笔记）→ 点击走对话流程");
} else {
  const hint = await ev(
    `document.querySelector('.plugin-ui-view .ai-empty .ai-hint')?.textContent`,
  );
...
await waitFor(
  `!!document.querySelector('.plugin-ui-view .ai-msg.ai-msg-assistant.err')`,
  "AI 错误消息出现（未配置 API Key）",
);
const errText = await ev(
  `document.querySelector('.plugin-ui-view .ai-msg.ai-msg-assistant.err .ai-msg-content')?.textContent`,
);
...
await waitFor(
  `[...document.querySelectorAll('.plugin-ui-view .ai-msg.ai-msg-user')].some(m => m.textContent.includes('基于笔记检索'))`,
  "RAG 用户消息出现",
);
await waitFor(
  `document.querySelectorAll('.plugin-ui-view .ai-msg.ai-msg-assistant.err').length >= 2`,
  "RAG 回复（错误路径）出现",
);
