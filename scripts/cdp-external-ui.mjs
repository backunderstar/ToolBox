...
const hasNav = await ev(
  `[...document.querySelectorAll('.nav-item')].some(e => e.textContent.trim() === '文本统计')`,
);
...
// ---- 3. 界面内实时统计 ----
const setText = await ev(
  `(() => { const ta = document.querySelector('.text-stats-input'); if (!ta) return false; const setter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value').set; setter.call(ta, '你好 world\\n第二行\\n\\n第三段'); ta.dispatchEvent(new Event('input', { bubbles: true })); return true; })()`,
);
if (!setText) throw new Error("无法写入统计输入框");
// UI 走 400ms 防抖后调 analyze 命令（api.call → 本地注册表），等足再断言
await sleep(700);
const vals = await ev(
  `(() => { const spans = [...document.querySelectorAll('.stat-value')]; return spans.map(s => s.textContent).join(','); })()`,
);
log(`PASS 界面内实时统计（字符/词/行/段 = ${vals}）`);
if (!vals.startsWith("17"))
  throw new Error(`字符数应含 17（你好 world\n第二行\n\n第三段）: ${vals}`);
...
const pyOk = await ev(
  `[...document.querySelectorAll('.plugin-card')].some(c => c.querySelector('.plugin-title h2')?.textContent.includes('Python 文本工具'))`,
);
...
await waitFor(
  `[...document.querySelectorAll('.plugin-card')].some(c => c.querySelector('.plugin-title h2')?.textContent.includes('Python 文本工具') && [...c.querySelectorAll('button')].some(b => b.textContent.trim() === '禁用'))`,
  "py-tools 已启用",
  15000,
);
...
if (!callRes.includes("2024/03/01"))
  throw new Error(`pytext.humanDate 应返回格式化日期: ${callRes}`);
