...
const hostView = await ev(
  `!!document.querySelector('.projects-view:not(.plugin-ui-view .projects-view)')`,
);
...
// ---- 2. 列表经桥加载（既有项目）----
await waitFor(
  `document.querySelectorAll('.plugin-ui-view .project-card').length >= 1`,
  "项目列表出现",
);
const names = await ev(
  `[...document.querySelectorAll('.plugin-ui-view .project-card-name')].map(e => e.textContent)`,
);
...
await waitFor(
  `!!document.querySelector('.plugin-ui-view .project-detail')`,
  "项目详情（文件浏览器）",
);
