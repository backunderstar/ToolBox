...
for (let i = 0; i < 6 && !floatConn; i++) {
  ...
  if (f) { floatConn = { conn: await connect(f), target: f }; break; }
...
// ---- 3. 插件自带前端挂载（标题栏/输入行）----
await waitFor(floatConn.conn.ev, `!!document.querySelector('.float-window .float-titlebar')`, "浮窗插件 UI 标题栏");
await waitFor(floatConn.conn.ev, `!!document.querySelector('.float-window .float-input')`, "浮窗插件 UI 输入行");
